...
package mr.iplocation;
import mr.iplocation.IPEntry;
...
public class IPSeeker {
		...
		for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
			...
			if (temp != -1) {
				...
				if (loc.country.indexOf(s) != -1 || loc.area.indexOf(s) != -1) {
					...
					// 得到起始IP
					readIP(offset - 4, b4);
					entry.beginIp = org.dataguru.mr.iplocation.Utils.getIpStringFromBytes(b4);
					// 得到结束IP
					readIP(temp, b4);
					entry.endIp = org.dataguru.mr.iplocation.Utils.getIpStringFromBytes(b4);
					// 添加该记录
					ret.add(entry);
				}
			}
	...
	public String getCountry(byte[] ip) {
		...
		if (ipFile == null)
			return "错误的IP数据库文件";
		// 保存ip，转换ip字节数组为字符串形式
		String ipStr = org.dataguru.mr.iplocation.Utils.getIpStringFromBytes(ip);
	...
	public String getCountry(String ip) {
		return getCountry(org.dataguru.mr.iplocation.Utils.getIpByteArrayFromString(ip));
	}
	...
	public String getArea(String ip) {
		return getArea(org.dataguru.mr.iplocation.Utils.getIpByteArrayFromString(ip));
	}
	...
	private String readString(long offset) {
		try {
			...
			for (i = 0, buf[i] = ipFile.readByte(); buf[i] != 0; buf[++i] = ipFile
				...
				;
			if (i != 0)
				return org.dataguru.mr.iplocation.Utils.getString(buf, 0, i, "GBK");
		} catch (IOException e) {
			System.out.println(e.getMessage());
		}
		return "";
